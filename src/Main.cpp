...
int main(int argc, char* argv[]) {
    ...
    for(const auto& modpath : vfs.modpaths | std::views::reverse) {
        if(fsconfig(fsfd, FSCONFIG_SET_STRING, "lowerdir+", modpath.c_str(), 0) < 0) {
            std::cout << "Failed to set VFS lowerdir: " << fsfd << std::endl;
            return 1;
        }
    }
    if(fsconfig(fsfd, FSCONFIG_SET_STRING, "lowerdir+", vfs.gameroot.c_str(), 0) < 0) {
        std::cout << "Failed to set VFS lowerdir: " << fsfd << std::endl;
        return 1;
    }
    if(fsconfig(fsfd, FSCONFIG_SET_STRING, "upperdir", vfs.overwrite.c_str(), 0) < 0) {
    ...
    move_mount(mntfd, "", AT_FDCWD, vfs.gameroot.c_str(), MOVE_MOUNT_F_EMPTY_PATH);
    return 0;
}
